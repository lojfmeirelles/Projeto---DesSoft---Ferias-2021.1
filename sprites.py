import random
import pygame
from config import WIDTH, HEIGHT, METEOR_WIDTH, METEOR_HEIGHT, SHIP_WIDTH, SHIP_HEIGHT, LIFE_HEIGHT, LIFE_WIDTH
from assets import SHIP_IMG, PEW_SOUND, METEOR_IMG, BULLET_IMG, EXPLOSION_ANIM, SHIP2_IMG, METEOR2_IMG, LIFE_IMG
...
class Second_Meteor(pygame.sprite.Sprite):
    def __init__(self, assets):
        # Construtor da classe mãe (Sprite).
        pygame.sprite.Sprite.__init__(self)
...
class Life(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self.rect.x += self.speedx
        self.rect.y += self.speedy
        # Se a vida passar do final da tela, volta para cima e sorteia
        # novas posições e velocidades
        if self.rect.top > HEIGHT or self.rect.right < 0 or self.rect.left > WIDTH:
            self.rect.x = random.randint(0, WIDTH-LIFE_WIDTH)
            self.rect.y = random.randint(-100, -LIFE_HEIGHT)
